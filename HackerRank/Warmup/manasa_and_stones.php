...
$inputFile = fopen("manasa_and_stones.txt", "r") or die("Unable to open file!");
...
$j = 0;
for ($i = 0; $i < $data[0]; $i++) {
    echo manasa($data[$j+1], $data[$j+2], $data[$j+3]), "\n";
    $j = $j + 3;
}

function manasa($stones, $a, $b)
{
    $last1  = ($stones * $a) - $a;
    $last2  = ($stones * $b) - $b;
    $step   = abs($b - $a);
    $result = array($last1, $last2);
    $start  = ($last1 > $last2) ? $last2 : $last1;
    $end    = ($last1 > $last2) ? $last1 : $last2;
